...
[FIX] 1) auto_profile เดาโปรไฟล์การไต่ + closed-loop guidance
      2) Stage.ignition_delay รองรับ coast ก่อนจุดจรวดท่อนบน
      3) fairing_jettison_t / payload_deploy_t รองรับ timeline ภารกิจจริง
      4) orbital_elements ใช้ semi-latus rectum แก้ perigee/apogee ติดลบ
"""
...
class Vehicle:
    ...
    fairing_jettison_t: Optional[float] = None
    payload_deploy_t: Optional[float] = None
...
class Mission:
    ...
    mission_type: str = "circular"
    target_apogee_alt: float = 500e3
...
# ============================================================
# AUTO ASCENT PROFILE  (ใหม่ — แก้ปัญหาไม่รู้โปรไฟล์การไต่)
# ============================================================
def auto_ascent_profile(vehicle: Vehicle, mission: Mission) -> Tuple[float, float, float]:
    m0 = max(vehicle.gross_mass(), 1.0)
    F0 = max(vehicle.stages[0].thrust_sl, 1.0)
    twr0 = float(np.clip(F0 / (m0 * G0), 1.05, 3.0))
    excess = twr0 - 1.0
    vt = float(np.clip(6.0 / (twr0 - 0.90), 2.0, 25.0))
    ka = float(np.clip(2.0 + 3.5 * excess, 1.5, 9.0))
    kd = float(np.clip(6.0 + 6.0 * excess, 4.0, 25.0))
    return vt, ka, kd
...
def simulate_ascent(vehicle: Vehicle, mission: Mission,
    ...
    # --- โปรไฟล์การไต่ ---
    if mission.auto_profile:
        vert_t, kick_ang, kick_dur = auto_ascent_profile(vehicle, mission)
    else:
        vert_t = mission.vertical_time
        kick_ang = mission.kick_angle
        kick_dur = mission.kick_duration

    # --- สภาวะเริ่มต้น ---
    r_ecef = geodetic_to_ecef(mission.site_lat, mission.site_lon, mission.site_alt)
    c, s = np.cos(theta0), np.sin(theta0)
    r = np.array([c * r_ecef[0] - s * r_ecef[1],
                  s * r_ecef[0] + c * r_ecef[1],
                  r_ecef[2]])
    v = np.cross(np.array([0.0, 0.0, OMEGA_E]), r)
    state = np.concatenate([r, v])

    stages = vehicle.stages
    prop_left = [sg.prop_mass for sg in stages]
    ...
    def current_mass():
        ...
        for i in range(idx, len(stages)):
            m += stages[i].dry_mass + prop_left[i]
        return m

    def steering(rv, tt, burning):
        rr, vv = rv[0:3], rv[3:6]
        e, n, up = enu_basis(rr)
        ...
        if not burning:
            return v_rel / nv if nv > 1e-6 else up
        if tt < vert_t:
            return up
        if tt < vert_t + kick_dur:
            f = (tt - vert_t) / max(kick_dur, 1e-6)
            p = kick_ang * D2R * f
            return np.cos(p) * up + np.sin(p) * horiz
        if apo_lock:
            vh = vv - np.dot(vv, up) * up
            nvh = np.linalg.norm(vh)
            return vh / nvh if nvh > 1.0 else horiz
...
# === main loop ===
    while t < max_time:
        ...
        if vehicle.payload_deploy_t is not None and t >= vehicle.payload_deploy_t:
            break

        if fairing_on:
            if vehicle.fairing_jettison_t is not None:
                if t >= vehicle.fairing_jettison_t:
                    fairing_on = False
            elif alt >= vehicle.fairing_jettison_alt:
                fairing_on = False
        ...
        if (idx < len(stages) and not burning and t >= ignite_at
                and (prop_left[idx] <= 0.0
                     or stage_time[idx] >= stages[idx].burn_time - 1e-9)):
            if idx + 1 < len(stages):
                idx += 1
                ignite_at = t + max(stages[idx].ignition_delay, 0.0)
                continue
            else:
                if vehicle.payload_deploy_t is not None and t <= vehicle.payload_deploy_t:
                    idx = len(stages)
        ...
        if not apo_lock and alt > 50000.0:
            if apoapsis_radius(state[0:3], state[3:6]) >= target_r:
                apo_lock = True

        rho, p, T_atm, a_snd = atmosphere(alt)
        mass = current_mass()
        u = steering(state, t, burning)
        ...
        if t >= next_out - 1e-9:
            v_rel = state[3:6] - np.cross(np.array([0.0, 0.0, OMEGA_E]), state[0:3])
            vr = np.linalg.norm(v_rel)
            up = state[0:3] / rn
            fpa = np.degrees(np.arcsin(np.clip(
                np.dot(v_rel, up) / max(vr, 1e-6), -1, 1)))
            rows.append(dict(
        ...
        upper = (idx >= 1) or (len(stages) == 1)
        if burning and upper and alt > 60000.0:
            r_now, v_now = state[0:3], state[3:6]
            energy = 0.5 * float(np.dot(v_now, v_now)) - MU / np.linalg.norm(r_now)
            if energy >= 0.0:
                break
            apo = apoapsis_radius(r_now, v_now)
            if apo >= target_r:
                peri = periapsis_radius(r_now, v_now)
                if mission.mission_type == "elliptical":
                    if peri >= RE + mission.target_alt:
                        break
                elif peri >= RE + mission.target_alt - 5000.0:
                    break

        f_rk = lambda y: deriv(y, mass, thr_vec, alt)
        k1 = f_rk(state)
        k2 = f_rk(state + 0.5 * dt * k1)
        k3 = f_rk(state + 0.5 * dt * k2)
        k4 = f_rk(state + dt * k3)
...
def coast_and_circularize(state, t_end, target_alt, mission_type="circular",
                          dt=1.0, max_coast=28800.0, verbose=True):
    state = np.array(state, dtype=float)
    if mission_type == "escape":
        return state, t_end, 0.0
    if mission_type == "elliptical":
        return state, t_end, 0.0
    if apoapsis_radius(state[0:3], state[3:6]) < RE + 120e3:
        raise ValueError("พลังงานไม่พอเข้าวงโคจร: apoapsis ต่ำกว่า ~120 km")
    y = state.copy()
    t = 0.0
    ...
    if verbose:
        print(f"Circularize @ {(rn-RE)/1000:.1f} km  dv={dv:.0f} m/s  coast={t:.0f}s")
    return st6, t_end + t, dv

     # ============================================================
# ORBITAL ELEMENTS  (แก้ perigee/apogee ติดลบ)
# ============================================================
def orbital_elements(r, v) -> Dict[str, float]:
    ...
    if e > 1e-9:
        ...
        ta = np.arctan2(np.dot(r, np.cross(h, n)) / hn,
                        np.dot(r, n)) if nn > 1e-9 else 0.0

    return dict(
        a_km=a / 1000.0, e=e, inc_deg=float(inc * R2D),
        raan_deg=float(raan * R2D % 360.0),
        argp_deg=float(argp * R2D % 360.0),
        ta_deg=float(ta * R2D % 360.0),
        perigee_km=(perigee_r - RE) / 1000.0,
        apogee_km=apogee_km,
        period_min=period_min, v_now_mps=float(vn),
...
def look_angles(df, obs_lat, obs_lon, obs_alt, launch_jd, theta0):
    ...
    for row in df.itertuples():
        ...
        sun_el = np.degrees(np.arcsin(np.clip(
            np.dot(sun, obs / np.linalg.norm(obs)), -1, 1)))
    ...
    if len(out) > 1:
        ...
        out['angular_rate'] = np.degrees(
            np.sqrt((du_ * cs) ** 2 + de_ ** 2)) / np.maximum(dt_, 1e-6)
    else:
